...
# augix, it seems in transformers version 4.30.2, the following import is correct.
...
class LongformerSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        if config.num_hiddens % config.num_heads != 0:
            raise ValueError(
                "The hidden size (%d) is not a multiple of the number of attention "
                "heads (%d)" % (config.num_hiddens, config.num_heads))
        self.num_heads = config.num_heads
        self.head_dim = int(config.num_hiddens / config.num_heads)
        self.embed_dim = config.num_hiddens

        self.query = nn.Linear(config.num_hiddens, self.embed_dim)
        self.key = nn.Linear(config.num_hiddens, self.embed_dim)
        self.value = nn.Linear(config.num_hiddens, self.embed_dim)

        self.query_global = nn.Linear(config.num_hiddens, self.embed_dim)
        self.key_global = nn.Linear(config.num_hiddens, self.embed_dim)
        self.value_global = nn.Linear(config.num_hiddens, self.embed_dim)
